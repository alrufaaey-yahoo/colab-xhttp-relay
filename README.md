...
4.  **تثبيت الاعتمادات:** لا توجد اعتمادات Node.js إضافية مطلوبة لـ `localhost.run`، حيث يتم استخدامه عبر SSH.

    ```bash
    !npm install
    ```

5.  **تشغيل الخادم:** قم بتشغيل الخادم. سيقوم `localhost.run` بإنشاء نفق SSH تلقائيًا لتطبيقك.

    ```bash
    !node index.js
    ```

    بعد تشغيل الخادم، ستحصل على رابط `localhost.run` عام في مخرجات الخلية (ابحث عن سطر يبدأ بـ `✅ Tunnel established!`). استخدم هذا الرابط للوصول إلى تطبيق الترحيل الخاص بك.
