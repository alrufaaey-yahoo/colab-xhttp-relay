...
4.  **تثبيت الاعتمادات:** لا توجد اعتمادات Node.js إضافية مطلوبة لـ `Serveo (serveo.net)`، حيث يتم استخدامه عبر SSH.

    ```bash
    !npm install
    ```

5.  **تشغيل الخادم:** قم بتشغيل الخادم. سيقوم `Serveo (serveo.net)` بإنشاء نفق SSH تلقائيًا لتطبيقك.

    ```bash
    !node index.js
    ```

    بعد تشغيل الخادم، ستحصل على رابط `Serveo (serveo.net)` عام في مخرجات الخلية (ابحث عن سطر يبدأ بـ `✅ Tunnel established!`). استخدم هذا الرابط للوصول إلى تطبيق الترحيل الخاص بك.
